...
const PartnerOption: React.FC<{ option: any, index: number, onNavigate?: (page: Page, sectionId?: string) => void }> = ({ option, index, onNavigate }) => {
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    card.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) ${option.highlight ? 'scale(1.05)' : ''}`;
  };

  const handleMouseLeave = (e: React.MouseEvent<HTMLDivElement>) => {
    e.currentTarget.style.transform = `perspective(1000px) rotateX(0deg) rotateY(0deg) ${option.highlight ? 'scale(1.05)' : ''}`;
  };
  ...
  return (
    <div
      ...
      className={`relative bg-[#280c2d]/90 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-10 flex flex-col items-center text-center border transition-all duration-300 tilt-card shadow-xl overflow-visible ${
        option.highlight 
          ? 'border-[#fae78e] scale-[1.05] z-20 shadow-[0_0_50px_rgba(250,231,142,0.15)] my-6 md:my-0' 
          : 'border-[#fae78e]/10 group hover:border-[#fae78e]/50'
      }`}
    >
      {option.highlight && (
...
const PartnerSection: React.FC<PartnerSectionProps> = ({ onNavigate }) => {
  ...
  return (
    <section id="partner" className="py-24 md:py-32 bg-black transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16 items-center perspective-1000">
          {partnershipOptions.map((option, index) => (
            <div key={index} className="animate-in fade-in slide-in-from-bottom-8" style={{ animationDelay: `${index * 150}ms` }}>
              <PartnerOption option={option} index={index} onNavigate={onNavigate} />
            </div>
          ))}
        </div>

        <div className="text-center mt-24 text-slate-400 font-serif-display text-lg md:text-xl animate-in fade-in duration-700 delay-500">
