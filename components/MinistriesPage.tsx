    ...
    <div className="bg-black text-white font-sans selection:bg-[#fae78e] selection:text-[#280c2d]">
      <section className="relative pt-20 pb-16 md:pt-32 md:pb-24 overflow-hidden min-h-[60vh] flex items-center">
        <div className="absolute inset-0 z-0">
             <img 
                src="https://bezaleelforge.github.io/kachi/13.jpg" 
      ...
      <section className="py-16 bg-[#0a0a0a] border-b border-[#fae78e]/10">
         <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="relative aspect-video rounded-3xl overflow-hidden shadow-[0_0_40px_rgba(40,12,45,0.6)] border border-[#fae78e]/20 group">
                <img src="https://bezaleelforge.github.io/kachi/18.jpg" className="w-full h-full object-cover opacity-70 group-hover:opacity-50 transition-opacity duration-500" alt="Ministry Video" />
      ...
      <section id="ministry-details" className="py-20 bg-black relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div key={activeTab} className="bg-[#1a1a1a] rounded-[3rem] shadow-2xl border border-[#fae78e]/10 overflow-hidden animate-in fade-in slide-in-from-right-8 duration-700 ease-out">
               <div className="flex flex-col lg:flex-row">
                 <div className="lg:w-2/5 p-8 md:p-12 bg-[#280c2d] border-b lg:border-b-0 lg:border-r border-[#fae78e]/10 relative overflow-hidden flex flex-col">
                    <div className="absolute top-0 right-0 w-full h-full">
                       <img src={ministryDetails[activeTab].image} className="w-full h-full object-cover opacity-10 blur-sm" alt="" />
                    </div>
                    <div className="w-16 h-16 rounded-2xl bg-[#fae78e] flex items-center justify-center mb-8 shadow-[0_0_20px_rgba(250,231,142,0.3)] animate-in zoom-in duration-500 delay-200 relative z-10">
                      {React.createElement(ministryDetails[activeTab].icon, { className: "w-8 h-8 text-[#280c2d]" })}
                    </div>
                    <h4 className="text-3xl font-display font-bold text-white mb-2 relative z-10">{ministryDetails[activeTab].title}</h4>
                    <p className="text-[#fae78e] font-bold uppercase text-xs mb-6 tracking-widest relative z-10">{ministryDetails[activeTab].subtitle}</p>
                    <p className="text-slate-300 leading-relaxed mb-10 text-lg italic font-light border-l-2 border-[#fae78e]/30 pl-4 relative z-10">
                    ...
                    <div className="mt-auto flex flex-col gap-6 relative z-10">
                      ...
                      <div className="pt-6 border-t border-[#fae78e]/10">
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => handleShare('facebook', ministryDetails[activeTab].title)} className="w-10 h-10 rounded-full bg-[#1877F2]/10 border border-[#1877F2]/30 flex items-center justify-center text-[#1877F2] hover:bg-[#1877F2] hover:text-white transition-all shadow-sm"><Facebook className="w-4 h-4" /></button>
                          <button onClick={() => handleShare('twitter', ministryDetails[activeTab].title)} className="w-10 h-10 rounded-full bg-[#1DA1F2]/10 border border-[#1DA1F2]/30 flex items-center justify-center text-[#1DA1F2] hover:bg-[#1DA1F2] hover:text-white transition-all shadow-sm"><Twitter className="w-4 h-4" /></button>
                          <button onClick={() => handleShare('whatsapp', ministryDetails[activeTab].title)} className="w-10 h-10 rounded-full bg-[#25D366]/10 border border-[#25D366]/30 flex items-center justify-center text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all shadow-sm"><MessageCircle className="w-4 h-4" /></button>
                        </div>
                      </div>
                    </div>
                 </div>
            ...
            <div className="relative bg-[#280c2d] rounded-[3rem] p-6 md:p-12 border border-[#fae78e]/10 shadow-[0_0_50px_rgba(40,12,45,0.5)] overflow-hidden min-h-[500px] flex items-center justify-center">
                ...
                <WorldMapSVG>
                    {impactedCountries.map((country) => (
                      <g key={country.id} onMouseEnter={() => setHoveredCountry(country.id)} onMouseLeave={() => setHoveredCountry(null)} className="cursor-pointer">
      ...
      <section className="py-20 bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {featured.map((item, i) => (
                    <div key={i} className="group bg-[#280c2d] p-8 rounded-3xl border border-[#fae78e]/10 hover:border-[#fae78e]/50 transition-all duration-500 shadow-lg hover:shadow-[0_0_30px_rgba(40,12,45,0.8)] hover:-translate-y-2 relative overflow-hidden">
                        ...
                        <div className="flex items-start justify-between mb-6">
                            <div className={`p-4 rounded-2xl bg-[#000] text-[#fae78e] shadow-inner group-hover:scale-110 transition-transform border border-[#fae78e]/20`}><item.icon className="w-8 h-8 drop-shadow-[0_0_5px_rgba(250,231,142,0.5)]" /></div>
                        ...
                        <button onClick={() => { const target = document.getElementById('ministry-details'); target?.scrollIntoView({ behavior: 'smooth' }); setActiveTab(i); }} className="flex items-center gap-2 text-white/70 hover:text-[#fae78e] font-bold hover:gap-4 transition-all text-sm focus-visible:ring-2 focus-visible:ring-[#fae78e] focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-md">Deep Dive <ArrowRight className="w-4 h-4" /></button>
                    </div>
                ))}
            </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {arms.map((arm, i) => (
                    <div key={i} className="bg-black/40 backdrop-blur-md p-8 rounded-3xl border border-[#fae78e]/10 hover:border-[#fae78e]/40 transition-all duration-300 flex flex-col items-center text-center group hover:-translate-y-2 shadow-xl">
                        <div className="w-16 h-16 rounded-full bg-[#fae78e] flex items-center justify-center mb-6 shadow-[0_0_20px_#fae78e] group-hover:rotate-12 transition-transform"><arm.icon className="w-8 h-8 text-[#280c2d]" /></div>
                        ...
                        <button className="px-8 py-2.5 bg-transparent border border-[#fae78e] text-[#fae78e] font-bold rounded-full hover:bg-[#fae78e] hover:text-[#280c2d] transition-all shadow-[0_0_10px_rgba(250,231,142,0.2)] focus-visible:ring-2 focus-visible:ring-[#fae78e] focus-visible:ring-offset-2 focus-visible:ring-offset-black">{arm.btn}</button>
                    </div>
                ))}
            </div>
        </div>
