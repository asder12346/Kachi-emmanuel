...
const Navbar: React.FC<NavbarProps> = ({ isDarkMode, toggleDarkMode }) => {
  ...
  const navLinks = [
    ...
    { name: 'Partner With Us', href: '#partner', icon: Heart },
    ...
    <header className="sticky top-0 z-50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-lg border-b border-slate-200/60 dark:border-white/10 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <a href="#home" className="flex items-center gap-2 group">
          ...
          <span className="font-display font-bold text-xl md:text-2xl tracking-tight text-slate-900 dark:text-white leading-none">
          ...
          <nav className="flex flex-col gap-6 text-xl">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={() => setIsMenuOpen(false)}
                className="flex items-center gap-4 text-slate-700 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-300 font-semibold transition-colors py-2 border-b border-slate-100 dark:border-slate-800 last:border-b-0"
              >
